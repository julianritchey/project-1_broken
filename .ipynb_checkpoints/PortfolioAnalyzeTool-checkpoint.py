...
def calculate_sharpe_ratios(symbols, start_date, end_date, index='^GSPC'):
    ...
    for symbol in symbols:
        ...
        sharpe_ratios_index = round((combined_returns.iloc[:, -1].mean()) / (combined_returns.iloc[:, -1].std() * np.sqrt(252)),4)
